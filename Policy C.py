    ...
    for alpha in Alpha:
        ...
        for i in range (1,25):      # time
            for j in range (0,11):  # Soc
                ...
                # penalty on charging/discharging & no refunds on negative grid energy   
                # grid_energy=max(demand_quantile[k][i-1]+max(alpha*eta,alpha/eta)*Cb,0)
                
                # no penalty on charging/discharging but refunds on negative grid energy 
                # grid_energy=demand_quantile[k][i-1]+alpha*Cb
                
                
                # penalty on charging/discharging but refunds on negative grid energy   
                grid_energy=demand_quantile[k][i-1]+max(alpha*eta,alpha/eta)*Cb
                ...
                C_[k,jj,i-1,j]=np.round(cost*grid_energy+penalty,1) 
...
for jj in range(255):    # one year
    for i in range (24):
        ...
        # penalty on charging/discharging & no refunds on negative grid energy   
        # current_cost=cost*(max(point+max(alpha*eta,alpha/eta)*Cb,0))
        ...
        # penalty on charging/discharging but refunds on negative grid energy   
        current_cost=cost*(point+max(alpha*eta,alpha/eta)*Cb)
        demnad_after[ii]=point+max(alpha*eta,alpha/eta)*Cb
...
for l in range(24):
    ...
    # # you can add this if you want to see the saving at certian interval try with jj=35
    # if l<11:
    #     continue
    # if l>16:
    #     continue
    ...
    demand=school_test_work[24*jj+l]
    if demand>=200:
        cost=0.12
    else:
        cost=0.05
    cost_total=cost_total+demand*cost



    demnad_after_= demnad_after[24*jj+l]

    if demnad_after_>=200:
        cost=0.12
    else:
        cost=0.05
    cost_total_after=cost_total_after+demnad_after_*cost

saving=cost_total-cost_total_after
...
B=m.addVars(24*255, vtype=GRB.BINARY, name='y')
Z=m.addVars(24*255, vtype='C',lb=-1, ub=1, name='y')


obj=gp.quicksum(0.05*(school_test_work[i]+Cb*u[i]) + 0.07*(school_test_work[i]*B[i]+Cb*Z[i]) for i in  range(24*255) )
m.setObjective(obj) # the defult is to MINIMIZE
...
m.addConstrs(Z[i]>=-B[i] for i in  range(24*255) ) # 
m.addConstrs(Z[i]<=B[i] for i in  range(24*255) ) # 
m.addConstrs(u[i]-Z[i]>=-1+B[i] for i in  range(24*255) ) #
m.addConstrs(u[i]-Z[i]<=1-B[i] for i in  range(24*255) ) #
m.addConstrs(Eg[i]>=200-M*(1-B[i]) for i in  range(24*255) ) # 
m.addConstrs(Eg[i]<=200+M*B[i] for i in  range(24*255) ) # 
...
u=m.addVars(24*255, vtype='C',lb=-1, ub=1, name='y')
Es=m.addVars(24*255, vtype='C',lb=0, ub=Cb, name='y')
Eg=m.addVars(24*255, vtype='C',lb=0, ub=GRB.INFINITY, name='y')
Eb=m.addVars(24*255, vtype='C',lb=-Cb, ub=Cb, name='y')
# aux=m.addVars(24*255, vtype='C',lb=-Cb*eta, ub=Cb*eta, name='y')
# aux2=m.addVars(24*255, vtype='C',lb=-Cb/eta, ub=Cb/eta, name='y')

B=m.addVars(24*255, vtype=GRB.BINARY, name='y')
Z=m.addVars(24*255, vtype='C',lb=-Cb, ub=Cb, name='y')


obj=gp.quicksum(0.05*(school_test_work[i]+Eb[i]) + 0.07*(school_test_work[i]*B[i]+Z[i]) for i in  range(24*255) )
m.setObjective(obj) # the defult is to MINIMIZE


# m.addConstrs(aux[i]==Cb*u[i]*eta for i in  range(24*255))
# m.addConstrs(aux2[i]==Cb*u[i]/eta for i in  range(24*255))
# m.addConstrs(Eb[i]==gp.max_(aux[i], aux2[i]) for i in  range(24*255))

m.addConstrs(Eb[i]>=Cb*u[i]*eta  for i in  range(24*255))
m.addConstrs(Eb[i]>=Cb*u[i]/eta  for i in  range(24*255))


m.addConstrs(Eg[i]==school_test_work[i]+Eb[i] for i in  range(24*255) ) # 
...
m.addConstrs(Z[i]>=-Cb*B[i]*eta for i in  range(24*255) ) # 
m.addConstrs(Z[i]<=Cb*B[i]/eta for i in  range(24*255) ) # 
m.addConstrs(Eb[i]-Z[i]>=-eta*Cb*(1-B[i]) for i in  range(24*255) ) #
m.addConstrs(Eb[i]-Z[i]<=Cb*(1-B[i])/eta for i in  range(24*255) ) #
m.addConstrs(Eg[i]>=200-M*(1-B[i]) for i in  range(24*255) ) # 
m.addConstrs(Eg[i]<=200+M*B[i] for i in  range(24*255) ) # 
