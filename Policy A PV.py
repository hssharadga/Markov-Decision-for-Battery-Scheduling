...
# plt.plot(school_train_work)
# plt.plot(school_train_work-1000/1000*np.array(PV[0:255*24]))
    ...
    for alpha in Alpha:
        ...
        for i in range (1,25):      # time
            for j in range (0,11):  # Soc
                ...
                if Es<0:
                    penalty=100000
                C_[k,jj,i-1,j]=np.round(cost[i-1]*(max(demand_quantile[k][i-1]+max(alpha*eta,alpha/eta)*Cb,0)) +penalty,1) # penalty on charging/discharging  and no refunds on negative grid energy 
                # C_[k,jj,i-1,j]=np.round(cost[i-1]*(demand_quantile[k][i-1]+alpha*Cb) +penalty,1) # no penalty on charging/discharging  but refunds on negative grid energy  
                
                # C_[k,jj,i-1,j]=np.round(cost[i-1]*(demand_quantile[k][i-1]+max(alpha*eta,alpha/eta)*Cb) +penalty,1) # penalty on charging/discharging but  refunds on negative grid energy
...
jj=random.randint(0,254)  # Day index out of 255 work days in this year
...
aux=m.addVars(24*255, vtype='C',lb=-eta, ub=eta, name='y')
aux2=m.addVars(24*255, vtype='C',lb=-eta, ub=eta, name='y')
