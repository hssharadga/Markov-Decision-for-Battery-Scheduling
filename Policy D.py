...
#############
Cb=500 # Battery
Peak=[0,100,200,300,400,500]
C_=np.zeros((9,21,24,11,6))   # Cost [t, Soc, demnad_quantile, Alpha,peak]  that what it is supposed to be
...
y=m.addVars(24, 11, 9, 21, 6, vtype='C',lb=0, ub=1, name='y')



obj=gp.quicksum(C_[q,k,i,j,o]*(1e-6)*y[i,j,q,k,o] for i in  range(24) for j in range (11) for q in range (9) for k in range (21)  for o in range (6))

m.setObjective(obj) # the defult is to MINIMIZE


m.addConstr(gp.quicksum(y[i,j,q,k,o]*(1e-6) for i in  range(24) for j in range (11) for q in range (9) for k in range (21)  for o in range (6))==1) 
...
m.addConstrs(gp.quicksum(y[ii,jj,qq,k,oo]*(1e-6) for k in range (21)) - gp.quicksum(y[i,j,q,k,o]*(1e-6)*(mm[q][qq]*SOC_p[k][j][jj]*t_p[i][ii]*peak_p[i][q][k][o][oo]) for k in range (21) for i in  range(24) for j in range (11) for q in range (9) for o in range (6))<=0  for ii in  range(24) for jj in range (11) for qq in range (9) for oo in range (6))
    ...
    for jj in range (11):
        for qq in range (9):
            for oo in range(6):
                ...
                p2= [ y[i,j,q,k,o].x*(mm[q][qq]*SOC_p[k][j][jj]*t_p[i][ii]*peak_p[i][q][k][o][oo]) for k in range (21) for i in  range(24) for j in range (11) for q in range (9) for o in range(6)]
                sum2=sum2+np.sum(p2)  
                cons=(sum1-sum2)
                # print(sum1)
                # print(sum2)
